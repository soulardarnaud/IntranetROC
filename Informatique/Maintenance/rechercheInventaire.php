...
$formulaire->FormulaireRechercheInventaire($inventaire);

if (isset($_POST['salle']) && isset($_POST['annee'])) {

    $inventaire = R::find('inventaire', 'salle_id=? and annee_id=?', array($_POST['salle'], $_POST['annee']));
    //var_dump($inventaire);
    ...
    if ($inventaire == !null) {
        ...
        $recherches.='</div><br>';
        $salle = R::findOne('salle', 'id=?', array($_POST['salle']));
        $anneeEnCours = R::findOne('annee', 'id=?', array($_POST['annee']));
        $anneeSuivante = R::findOne('annee', 'libelle>? ORDER BY libelle', array($anneeEnCours->libelle));

        $inventaireSuivant = R::find('inventaire', 'salle_id=? and annee_id=?', array($_POST['salle'], $anneeSuivante->id));
